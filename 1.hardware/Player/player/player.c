...
int g_device_mode = OFFLINE_MODE;
int g_sem_id = 0;
const char *path = "/tmp";
...
int InitSem() {
  ...
  s.val =
      1; // 初始值设置为1，表明一开始是允许进入临界区的，=0的时候就不能进入临界区了
  ...
  while (g_start_flag) {
    ...
    } else if (grand_pid == 0) // 孙进程
    {
      ...
      if (strlen(music_name) == 0) {
        if (FindNextMusic(cur_info.cur_mode, cur_info.cur_music, music_name) ==
            -1) {
          // 给父子进程发送信号
          // 父进程：重新请求一个新的歌曲链表
...
void NextPlay() {
  ...
  if (cur_info.cur_mode == CIRCLE) {
    ...
    printf("循环播放：%s\n", next);
  } else if (FindNextMusic(cur_info.cur_mode, cur_info.cur_music, next) ==
             -1) // 当前链表的5首已经都放完了
  {
    StopPlay();
    ClearMusicList();
...
void ChangeOfflineMode() {
  ...
  if(g_sockfd > 2)
  {
    ...
    g_maxfd = (g_sockfd == g_maxfd ? g_maxfd - 1 : g_maxfd);
    close(g_sockfd);
  ...
  StopPlay();
  g_start_flag = 0;
  g_suspend_flag = 0; // 主要是为了把g_suspend_flag置为0

  // 把U盘挂载到/mnt/usb，只有这样才能读取到U盘里面的数据
  char off_dev[64] = "/dev/sdc1";
  if (access(off_dev, F_OK) != 0) {
    memset(off_dev, 0, sizeof(off_dev));
    char bufa[64] = "/dev/sda1";
    memcpy(off_dev, bufa, sizeof(bufa));
    if (access(off_dev, F_OK) != 0) {
      memset(off_dev, 0, sizeof(off_dev));
      char bufb[64] = "/dev/sdb1";
      memcpy(off_dev, bufb, sizeof(bufb));
      if (access(off_dev, F_OK) != 0) {
        ReplyKeyWords("未检测到存储设备");
        return;
      }
    }
  }
  umount("/mnt/usb");
  mount(off_dev, "/mnt/usb", "exfat", 0, NULL);

  // 读取U盘的歌曲名，构建离线歌曲名链表

  GetMusicName("林俊杰");

  ReplyKeyWords("已经切换到离线模式");
}

void ChangeOnlineMode() {
  if (g_device_mode == ONLINE_MODE)
    return;
  StopPlay();
  g_start_flag = 0;
...
void ProcessAsrSignal(char *signal) {
  ...
  if (strstr(signal, "触发关键词")) {
    ...
    StartPlay();
  } else if (strstr(signal, "停止") || strstr(signal, "不想听") ||
             strstr(signal, "退出") || strstr(signal, "关闭")) {
    StopPlay();
  } else if (strstr(signal, "暂停") || strstr(signal, "休息") ||
             strstr(signal, "电话") || strstr(signal, "急事")) {
    SuspendPlay();
  } else if (strstr(signal, "继续") || strstr(signal, "恢复")) {
    ContinuePlay();
  } else if ((strstr(signal, "下") || strstr(signal, "换")) &&
             strstr(signal, "首")) {
    NextPlay();
  } else if ((strstr(signal, "上") || strstr(signal, "刚")) &&
             strstr(signal, "首")) {
    PrevPlay();
    ...
    GetMusicName("陈奕迅");
    StartPlay();
  } else if (strstr(signal, "林俊杰")) {
    StopPlay();
    GetMusicName("林俊杰");
    StartPlay();
  } else if (strstr(signal, "邓紫棋")) {
    StopPlay();
    GetMusicName("邓紫棋");
    StartPlay();
  } else if (strstr(signal, "换") && strstr(signal, "声音")) {
    ChangeVoive();
  } else if (strstr(signal, "离线模式")) {
    ChangeOfflineMode();
  } else if (strstr(signal, "在线模式")) {
    ChangeOnlineMode();
  } else {
    if (g_device_mode == OFFLINE_MODE) {
      ReplyKeyWords("请先连接网络后切换至在线模式");
    } else {
      char cmd[1024] = {0};
      sprintf(cmd, "/root/MusicPlayer/1.hardware/Chatmodel/build/qianwen %s",
              signal);
      system(cmd);
    }
  }
